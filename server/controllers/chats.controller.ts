...
class ChatsController {
    ...
    async getChatMedia(req: Request, res: Response) {
        const {userData, chatId} = req.body;
        const chat = await getChatById(chatId);
        const media = chat.messages.filter(msg => msg.type === "media")

        if (media.length) {
            return res.status(200).send({success: true, data: media});
        } else {
            return res.status(400).send({success: false, data: "Cannot Get Media"});
        }
    }
